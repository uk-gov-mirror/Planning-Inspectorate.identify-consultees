import type { ProjectGeometry, RulesetOption } from '../views/home/view-model.ts';
...
export const RULESETS: RulesetOption[] = [
	...
	{ value: 'post-30-apr-2024-england-wales', text: 'Post-30 April 2024, England and Wales' }
];
...
const DUMMY_PROJECTS: { reference: string; caseName: string }[] = [
	{ reference: 'EN010024', caseName: 'Navitus Bay Wind Park' },
...
export const DUMMY_GEOMETRIES: ProjectGeometry[] = Array.from({ length: 100 }, (_, index) => {
...
export function findDummyGeometry(geometryId: string): ProjectGeometry | undefined {
	return DUMMY_GEOMETRIES.find((geometry) => geometry.id === geometryId);
}

export function findRulesetLabel(rulesetValue: string): string {
	return RULESETS.find((ruleset) => ruleset.value === rulesetValue)?.text ?? rulesetValue;
}
